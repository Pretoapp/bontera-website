...
export async function generateStaticParams() {
  ...
  for (const locale of locales) {
    for (const project of projects ?? []) {
      if (!project?.slug) continue; // prevents crash
...
/* ═══════════════════════════════════════════════════════════════════════════
   LOCAL HELPERS (make sure we only render strings)
   ═══════════════════════════════════════════════════════════════════════════ */

function getProjectSizeLocalized(p: Project, locale: Locale): string {
  if (!p.size) return "";
  return typeof p.size === "string" ? p.size : (p.size[locale] ?? p.size.en);
}
...
export default async function ProjectDetailPage({ params }: Props) {
  ...
  // Scope (keep as-is; if your Project type does not include scope, TS may complain)
  const scope =
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (project as any).scope ? (((project as any).scope[loc] || (project as any).scope.en) as string) : "";

  // Always strings (no object rendering)
  const valueText = project.value ? getProjectValueLocalized(project, loc) : "";
  const durationText = project.duration ? getProjectDurationLocalized(project, loc) : "";
  const sizeText = getProjectSizeLocalized(project, loc);
  const featuresText = getProjectFeaturesLocalized(project, loc);

  // Get related projects
  const relatedProjects = getRelatedProjects(slug, project.category, 3);

  // Sidebar details (ALL values are strings)
  const projectDetails = [
    ...
    { key: "value", value: valueText, icon: "M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" },
    { key: "duration", value: durationText, icon: "M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" },
    { key: "size", value: sizeText, icon: "M3.75 3.75v4.5m0-4.5h4.5m-4.5 0L9 9M3.75 20.25v-4.5m0 4.5h4.5m-4.5 0L9 15M20.25 3.75h-4.5m4.5 0v4.5m0-4.5L15 9m5.25 11.25h-4.5m4.5 0v-4.5m0 4.5L15 15" },
...
{/* Breadcrumb (same flow as Quote page) */}
<nav className="mb-60" aria-label="Breadcrumb">
  <ol className="flex items-center gap-2 text-sm text-bontera-grey-400">
    ...
    <li className="text-white font-medium" aria-current="page">
      {title}
            ...
            {/* Quick Stats (strings only) */}
            {(valueText || sizeText || durationText) && (
              <div className="mt-10 flex flex-wrap gap-8 lg:gap-12">
                {valueText && (
                  <div>
                    <div className="text-3xl lg:text-4xl font-bold text-white">{valueText}</div>
                    <div className="text-sm text-bontera-grey-400 mt-1 uppercase tracking-wider">{t("details.value")}</div>
                  </div>
                )}
                {sizeText && (
                  <div>
                    <div className="text-3xl lg:text-4xl font-bold text-white">{sizeText}</div>
                    <div className="text-sm text-bontera-grey-400 mt-1 uppercase tracking-wider">{t("details.size")}</div>
                  </div>
                )}
                {durationText && (
                  <div>
                    <div className="text-3xl lg:text-4xl font-bold text-white">{durationText}</div>
...
<ProjectGallery
  images={(project.gallery ?? []).map((src: string, i: number) => ({
    src,
                ...
                return (
                  ...
                  >
                    <div className="relative aspect-[4/3] overflow-hidden">
                      ...
                      <div
                        className={`absolute top-4 ${isRTL ? "left-4" : "right-4"} w-10 h-10 bg-white/0 border border-white/30 flex items-center justify-center opacity-0 group-hover:opacity-100 group-hover:bg-white/20 transition-all duration-300`}
                      >
