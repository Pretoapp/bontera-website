...
// ═══════════════════════════════════════════════════════════════════════════
// PART 1 OF 2 - Copy this first, then add Part 2 below
// ═══════════════════════════════════════════════════════════════════════════
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // put this near your other hero vars
...
const STAIRS_ON_LEFT = false; // false = stairs on right, true = stairs on left
...
async function getFeaturedProjects(locale: string) {
  try {
    const featured = await client.fetch(featuredProjectsQuery, { locale });
    if (Array.isArray(featured) && featured.length > 0) return featured;
    const latest = await client.fetch(projectsQuery, { locale });
    return Array.isArray(latest) ? latest.slice(0, 6) : [];
  } catch (error) {
...
async function getLatestNews(locale: string): Promise<any[]> {
  try {
    const raw = await client.fetch(latestNewsQuery, { locale });
    if (!Array.isArray(raw)) return [];
    return raw.slice(0, 3).map((n: any, i: number) => {
      const slug = n?.slugCurrent ?? pickSlug(n?.slug);
      const img = n?.mainImage ?? n?.image ?? n?.coverImage;
      return {
        id: n?._id ?? `news-${i}`,
        title: pickLocalized(n?.title, locale, "News"),
        excerpt: pickLocalized(n?.excerpt, locale, ""),
        date: n?.publishedAt ?? n?._createdAt,
        href: slug ? `/${locale}/news/${slug}` : `/${locale}/news`,
        imageUrl: img ? urlFor(img).width(800).height(600).url() : "/images/placeholder.jpg",
      };
    });
  } catch (error) {
  ...
  const projects = (featuredProjects as FeaturedProject[])
    ...
    .map((p) => {
      ...
      return {
        ...
        category: pickLocalized(p.category, locale, "Construction"),
    ...
    const STAIRS_SIDE: "left" | "right" = "right"; // set "right" to match your request

const STAIRS = [
  // Tune these 4 rows to match your red drawing
  { top: "0%",  height: "34%", width: "62%" },
  { top: "34%", height: "22%", width: "46%" },
  { top: "56%", height: "22%", width: "72%" },
  { top: "78%", height: "22%", width: "55%" },
];
...
{/* Stepped navy overlay on the TEXT side (left), stepping right as it goes down */}
<div className="absolute inset-0 pointer-events-none z-[2]">
  {[
    // widths INCREASE downwards to match your red drawing direction
  ...
  {/* Soft fade on the opposite side (to the right) */}
...
{/* ═══════════════════════════════════════════════════════════════════════════
   PART 2 OF 2 - Paste this directly after Part 1 (inside the <main> tag)
═══════════════════════════════════════════════════════════════════════════ */}
        ...
        <div className="max-w-[1600px] mx-auto px-6 lg:px-16">
          ...
          {projects.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
              {projects[0] && (
                <Link href={projects[0].href} className="group relative md:col-span-2 lg:row-span-2 h-[400px] lg:h-auto min-h-[500px] overflow-hidden bg-bontera-grey-100">
                  <Image src={projects[0].imageUrl} alt={projects[0].title} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="66vw" />
                  ...
                  <div className="absolute inset-0 p-8 lg:p-12 flex flex-col justify-end">
                    <div className="flex items-center gap-4 mb-4">
                      <span className="px-3 py-1 bg-gray-500 text-white text-xs font-semibold uppercase tracking-wider">{projects[0].category}</span>
                      <span className="text-white/70 text-sm">{projects[0].year}</span>
                    </div>
                    <h3 className="text-3xl lg:text-4xl font-semibold text-white mb-3 group-hover:text-gray-300 transition-colors">{projects[0].title}</h3>
                    {projects[0].location && (
              ...
              {projects.slice(1, 4).map((project) => project && (
                <Link key={project.id} href={project.href} className="group relative h-64 lg:min-h-[240px] overflow-hidden bg-bontera-grey-100">
      ...
      ═══════════════════════════════════════════════════════════════════ */}
      <section className="relative py-28 lg:py-36 bg-bontera-grey-50 overflow-hidden">
        <div className="max-w-[1600px] mx-auto px-6 lg:px-16">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8 mb-16">
            <div>
              <span className="inline-flex items-center gap-3 text-bontera-navy-600 text-xs uppercase tracking-[0.3em] font-semibold">
              ...
              <h2 className="mt-6 text-4xl sm:text-5xl font-semibold text-bontera-grey-900 leading-[1.1] tracking-tight">{t("news.headline")}</h2>
            </div>
            <Link href={`/${locale}/news`} className="group inline-flex items-center gap-3 text-bontera-navy-600 hover:text-bontera-navy-700 text-sm uppercase tracking-wider font-semibold transition-colors">
              {t("news.viewAll")}
              <svg className={`w-5 h-5 transition-transform group-hover:translate-x-1 ${isRTL ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
              </svg>
          ...
          {latestNews.length > 0 ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {latestNews.map((news) => (
                <Link key={news.id} href={news.href} className="group bg-white border border-bontera-grey-200 hover:border-bontera-grey-300 hover:shadow-xl transition-all duration-300">
                  <div className="relative h-56 overflow-hidden">
                    <Image src={news.imageUrl} alt={news.title} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="33vw" />
                  </div>
                  <div className="p-6 lg:p-8">
                    {news.date && (
                      <time className="text-xs uppercase tracking-wider text-bontera-grey-500">
                        {new Date(news.date).toLocaleDateString(locale, { year: 'numeric', month: 'long', day: 'numeric' })}
                      </time>
                    )}
                    <h3 className="mt-3 text-xl font-semibold text-bontera-grey-900 group-hover:text-bontera-navy-600 transition-colors leading-tight">{news.title}</h3>
                    {news.excerpt && <p className="mt-3 text-sm text-bontera-grey-600 leading-relaxed line-clamp-2">{news.excerpt}</p>}
            ...
            <div className="text-center py-20 bg-white border border-bontera-grey-200">
              <p className="text-bontera-grey-600">{t("news.noNews")}</p>
            </div>
          )}
        </div>
      </section>
